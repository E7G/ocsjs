...
// @namespace https://www.greasyfork.us.kg/users/105582
// @downloadURL https://update.greasyfork.us.kg/scripts/396263/%E4%BC%98%E5%AD%A6%E9%99%A2-%E8%87%AA%E5%8A%A8%E6%8C%82%E6%9C%BA-%E5%80%8D%E9%80%9F-%E6%98%BE%E7%A4%BA%E7%AD%94%E6%A1%88.user.js
// @updateURL https://update.greasyfork.us.kg/scripts/396263/%E4%BC%98%E5%AD%A6%E9%99%A2-%E8%87%AA%E5%8A%A8%E6%8C%82%E6%9C%BA-%E5%80%8D%E9%80%9F-%E6%98%BE%E7%A4%BA%E7%AD%94%E6%A1%88.meta.js
// ==/UserScript==
...
(function () {
	...
	GM_JQ.innerHTML =
		"function showAnswer(){ var pageId = koLearnCourseViewModel.currentPage().id(); var sqList = []; var re = []; $('.question-wrapper').each(function(k,v){ var id = $(v).attr('id'); sqList.push(id.replace('question','')); }); $(sqList).each(function(k, id) { $.ajax({ async: false, type : \"get\", url : 'https://api.ulearning.cn/questionAnswer/' + id + '?parentId=' + pageId, datatype : 'json', success : function(result) { re.push(result.correctAnswerList); } }); }); var an = []; $(re).each(function(k,v){ an.push(v.join(',')); }); var t = $('.question-wrapper').find('.question-title-html'); t.each(function(k,v){ $(v).after('<span style=\"color:red;\">答案：'+an.shift()+'</span>'); }); }";
		...
		if (!submit_answer_button || submit_answer_button.innerHTML !== '提交') {
			...
			for (var i = 0; i < question.length; i++) {
				...
				$.ajax({
					...
					type: 'get',
					url:
						'https://api.ulearning.cn/questionAnswer/' + question[i].id.replace('question', '') + '?parentId=' + pageId,
					datatype: 'json',
	...
	$(unsafeWindow.document).ready(function () {
		...
		//删除指导元素
		document
			.getElementsByClassName('user-guide')[0]
			.parentNode.removeChild(document.getElementsByClassName('user-guide')[0]);
		//侧边栏
		var cbl = document.createElement('div');
		cbl.setAttribute(
			'style',
			'width: 60px;height: 74px;overflow: hidden;position: fixed;right: 10px;bottom: 10%;padding: 4px;background-color: #06a9f4;z-index: 9999;border: 1px solid rgb(233, 234, 236);'
		);
		...
		cbl.innerHTML =
			'<div style="cursor: pointer; text-align: center; padding: 0px;"><span id="set-auto" style="font-size: 12px;line-height: 16px;color: rgb(255, 255, 255);">设置<br />&<br />开关</span></div>';
				...
				gm_get('auto-exit').then((auto_exit) => {
					...
					$('#set-auto').click(function () {
						if ($('span[id="set-auto"]')[0].innerText == '设置\n&\n开关') {
							$('div[id="set-mune-hide"]').attr(
								'style',
								'position: fixed;height: 300px;bottom: 10%;z-index: 9999;right: 70px;'
							);
							$('div[id="set-mune-hide"]').attr('id', 'set-mune-unhide');
							$('span[id="set-auto"]')[0].innerHTML = '保存<br />&<br />隐藏';
						} else {
							$('div[id="set-mune-unhide"]').attr(
								'style',
								'position: fixed;height: 300px;bottom: 10%;z-index: 9999;right: 70px; display: none;'
							);
										...
										gm_get('auto-exit').then((auto_exit) => {
											if (document.getElementsByClassName('modal-backdrop fade in')[0] != null) {
												...
												if (document.querySelector("[data-bind='text: $root.i18nMsgText().gotIt']") != null) {
													...
													document.querySelector("[data-bind='text: $root.i18nMsgText().confirmLeave']") != null
												) {
													console.log('未完成确认');
													document.querySelector("[data-bind='text: $root.i18nMsgText().confirmLeave']").click();
												} else if (
													document.querySelector("[data-bind='text: i18nMessageText().nextChapter']") != null
												) {
													console.log('章节统计');
												...
												for (
													var count = 0;
													count < document.querySelectorAll("div[class='video-info'] span").length;
													count++
												) {
													var data_bind = document
														.querySelectorAll("div[class='video-info'] span")
														[count].getAttribute('data-bind');
												...
												if (all == true) {
													setTimeout(function () {
														...
														for (
															var k = 0;
															k < document.querySelectorAll("[data-bind='text: $root.nextPageName()']").length;
															++k
														) {
															console.log(
																document.querySelectorAll("[data-bind='text: $root.nextPageName()']")[k].innerHTML
															);
															if (
																document.querySelectorAll("[data-bind='text: $root.nextPageName()']")[k].innerHTML ==
																'没有了'
																...
																if (auto_exit == true) {
																	...
																	document.querySelector('button[id="startstop"]').innerHTML =
																		'学习完成（该按钮已不可点击）';
